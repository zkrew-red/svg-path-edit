...
export class SvgControlPoint extends SvgPoint {
  subIndex: number = 0;
  constructor(point: Point, public relations: Point[], movable = true) {
...
export abstract class SvgItem {
  ...
  public static MakeFrom(origin: SvgItem, previous: SvgItem, newType: string) {
    ...
    if (
      (origin instanceof CurveTo || origin instanceof SmoothCurveTo) &&
    ...
    if (
      (origin instanceof QuadraticBezierCurveTo ||
        origin instanceof SmoothQuadraticBezierCurveTo) &&
...
class CurveTo extends SvgItem {
  ...
  public refreshAbsoluteControlPoints(
    origin: Point,
    previousTarget: SvgItem | null
  ) {
    if (!previousTarget) {
      throw new Error('Invalid path');
    }
    this.absoluteControlPoints = [
      new SvgControlPoint(this.absolutePoints[0], [
...
class SmoothCurveTo extends SvgItem {
  ...
  public refreshAbsoluteControlPoints(
    ...
    this.absoluteControlPoints = [];
    if (
      previousTarget instanceof CurveTo ||
      previousTarget instanceof SmoothCurveTo
    ) {
      const prevLoc = previousTarget.targetLocation();
      const prevControl = previousTarget.absoluteControlPoints[1];
      const pts = new Point(
        2 * prevLoc.x - prevControl.x,
        2 * prevLoc.y - prevControl.y
      );
      this.absoluteControlPoints.push(
        new SvgControlPoint(pts, [prevLoc], false)
      );
    } else {
      const current = previousTarget
        ? previousTarget.targetLocation()
        : new Point(0, 0);
      const pts = new Point(current.x, current.y);
      this.absoluteControlPoints.push(new SvgControlPoint(pts, [], false));
    }
    this.absoluteControlPoints.push(
...
class SmoothQuadraticBezierCurveTo extends SvgItem {
  ...
  public refreshAbsoluteControlPoints(
    ...
    if (
      !(
        previousTarget instanceof QuadraticBezierCurveTo ||
        previousTarget instanceof SmoothQuadraticBezierCurveTo
      )
    ) {
...
class HorizontalLineTo extends SvgItem {
  ...
  public refreshAbsolutePoints(origin: Point, previous: SvgItem | null) {
    ...
    if (this.relative) {
      this.absolutePoints = [
        new SvgPoint(
          this.values[0] + this.previousPoint.x,
          this.previousPoint.y
        ),
      ];
    } else {
      this.absolutePoints = [
        new SvgPoint(this.values[0], this.previousPoint.y),
      ];
    }
  }
  public setTargetLocation(pts: Point) {
  ...
  public refreshAbsolutePoints(origin: Point, previous: SvgItem | null) {
    ...
    if (this.relative) {
      this.absolutePoints = [
        new SvgPoint(
          this.previousPoint.x,
          this.values[0] + this.previousPoint.y
        ),
      ];
    } else {
      this.absolutePoints = [
        new SvgPoint(this.previousPoint.x, this.values[0]),
      ];
    }
  }
  public setTargetLocation(pts: Point) {
...
class EllipticalArcTo extends SvgItem {
  ...
  public refreshAbsolutePoints(origin: Point, previous: SvgItem | null) {
    ...
    if (this.relative) {
      this.absolutePoints = [
        new SvgPoint(
          this.values[5] + this.previousPoint.x,
          this.values[6] + this.previousPoint.y
        ),
      ];
    } else {
...
export class Svg {
  ...
  asString(decimals: number = 4, minify: boolean = false): string {
    return this.path
      .reduce(
        (
          acc: { type?: string; item: SvgItem; trailing: SvgItem[] }[],
          it: SvgItem
        ) => {
