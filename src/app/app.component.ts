...
@Component({
  ...
  animations: [
    ...
    trigger('leftColumn', [
      ...
      transition(':leave', [
        animate('100ms ease', style({ 'max-width': '0' })),
      ]),
    ]),
  ],
})
export class AppComponent implements AfterViewInit {
  ...
  constructor(
    ...
    (window as any).browserComputePathBoundingBox =
      browserComputePathBoundingBox;
    for (const icon of [
      'delete',
      'logo',
      'more',
  ...
  @HostListener('document:keydown', ['$event']) onKeyDown($event: any) {
    ...
    if (tag !== 'INPUT' && tag !== 'TEXTAREA') {
      if (
        ...
        !$event.metaKey &&
        !$event.ctrlKey &&
        /^[mlvhcsqtaz]$/i.test($event.key)
      ) {
        const isLower = $event.key === $event.key.toLowerCase();
        const key = $event.key.toUpperCase();
        if (isLower) {
          // Item insertion
          const lastItem = this.parsedPath.path.length
            ? this.parsedPath.path[this.parsedPath.path.length - 1]
            : null;
          ...
          !isLower &&
          this.focusedItem &&
          this.canConvert(this.focusedItem, key)
        ) {
          // Item convertion
  ...
  pushHistory() {
    if (
      ...
      this.rawPath !== this.history[this.historyCursor]
    ) {
      this.historyCursor++;
      this.history.splice(
        this.historyCursor,
  ...
  canRedo(): boolean {
    return (
      this.historyCursor < this.history.length - 1 && !this.isEditingImages
    );
  }
  ...
  insert(type: string, after: SvgItem | null, convert: boolean) {
    if (convert) {
      if (after) {
        this.focusedItem = this.parsedPath.changeType(
          after,
          after.relative ? type.toLowerCase() : type
        );
  ...
  canInsertAfter(item: SvgItem | null, type: string): boolean {
    ...
    if (item !== null) {
      ...
      previousType = this.parsedPath.path[this.parsedPath.path.length - 1]
        .getType()
        .toUpperCase();
    }
    if (!previousType) {
